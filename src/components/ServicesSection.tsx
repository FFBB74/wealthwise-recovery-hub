...
const services = [
  ...
  },
  {
    ...
    icon: Search,
    description: "Rigorous investigation of scams, evidence gathering, and resolution for individuals and businesses.",
  },
  {
    title: "Wealth Management",
    icon: Lightbulb,
    description: "Personal financial advice to protect and grow your assets securely, tailored and ethical.",
  },
  {
    title: "Financial Consultancy",
    icon: ShieldCheck,
    description: "Practical planning guidance for better budgeting, investing, and reaching your goals.",
  },
  {
    title: "Scam Prevention Education",
    icon: GraduationCap,
    description: "Workshops, articles, and tips to help you confidently spot scams and avoid fraud.",
  },
  {
    title: "Digital Identity Verification",
    icon: ScanSearch,
    description: "Secure verification of identity/documents to make every transaction safer and worry-free.",
  },
]
  ...
  <section id="services" className="bg-white py-24 px-6">
    <div className="max-w-6xl mx-auto">
      ...
      <p className="text-center text-gray-600 mb-16 max-w-3xl mx-auto text-2xl leading-relaxed font-semibold animate-fade-in">
        <span className="text-brand-navy font-bold">Discover how WealthWise</span> recovers funds, investigates scams,
        and protects your financial future—with clarity and integrity.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 animate-fade-in">
        {services.map((service, i) => (
          <div
            key={service.title}
            className="bg-white/80 border border-brand-emerald/30 rounded-3xl shadow-2xl hover:shadow-emerald-200/70 transition-all duration-300 px-7 py-12 flex flex-col items-center text-center h-full group hover:bg-brand-gold/10 hover:border-emerald-300 animate-fade-in"
          >
            <img
              src={serviceImages[i] || "/placeholder.svg"}
              alt={service.title + " illustration"}
              className="w-32 h-32 object-cover rounded-2xl shadow-lg border mb-6 group-hover:scale-105 transition-transform duration-300"
            />
            <div className="bg-white rounded-xl p-4 mb-6 shadow-md group-hover:shadow-lg transition-shadow duration-300">
              <service.icon
                className="text-brand-emerald group-hover:text-brand-emerald-dark transition-colors duration-300"
                size={36}
              />
            </div>
